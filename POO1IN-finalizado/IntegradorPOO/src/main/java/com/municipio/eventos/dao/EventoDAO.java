...
public class EventoDAO {
    ...
    public boolean guardar(Evento evento) {
        ...
        try {
            ...
            System.out.println("DAO: Iniciando transacción para guardar evento: " + evento.getNombre() + " (ID: " + evento.getId() + ")");
            if (evento.getId() == null) { // Si es un nuevo evento
                em.persist(evento);
                System.out.println("DAO: Persistiendo nuevo evento: " + evento.getNombre() + " de tipo " + evento.getClass().getSimpleName());
            } else { // Si es un evento existente, lo fusiona (actualiza)
                em.merge(evento);
                System.out.println("DAO: Fusionando evento existente: " + evento.getNombre() + " (ID: " + evento.getId() + ") de tipo " + evento.getClass().getSimpleName());
            }
            em.getTransaction().commit();
            System.out.println("DAO: Transacción de guardado de evento completada con éxito para: " + evento.getNombre());
            return true;
        } catch (Exception e) {
            if (em.getTransaction().isActive()) {
                em.getTransaction().rollback();
                System.err.println("DAO: Transacción de guardado de evento revertida para: " + evento.getNombre());
            }
            System.err.println("ERROR CRÍTICO en EventoDAO.guardar() para evento '" + evento.getNombre() + "': " + e.getMessage());
