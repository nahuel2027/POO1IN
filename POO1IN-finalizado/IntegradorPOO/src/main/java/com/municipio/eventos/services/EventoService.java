...
import com.municipio.eventos.models.abstractas.Evento; // Importar Concierto
import com.municipio.eventos.models.enums.EstadoEvento;

import jakarta.persistence.EntityManager;
import jakarta.persistence.EntityManagerFactory;
import jakarta.persistence.EntityTransaction;
import jakarta.persistence.Persistence; // Importar Hibernate para inicialización explícita
...
public class EventoService {
    ...
    public boolean agregarEvento(Evento evento) {
        System.out.println("Service: Intentando agregar evento: " + evento.getNombre() + " de tipo " + evento.getClass().getSimpleName());
        Optional<Evento> existingEvent = eventoDAO.buscarPorNombreYFecha(evento.getNombre(), evento.getFechaInicio());
        if (existingEvent.isPresent()) {
            System.out.println("Service: Error: Ya existe un evento con el nombre '" + evento.getNombre() + "' para la fecha " + evento.getFechaInicio() + ". No se agregará duplicado.");
            return false;
        }
        try {
            if (evento instanceof Concierto) {
                Concierto concierto = (Concierto) evento;
                System.out.println("Service: Concierto a guardar. Artistas asociados: " + concierto.getArtistas().size());
                concierto.getArtistas().forEach(a -> System.out.println("  Service: - Artista: " + a.getNombreCompleto() + " (DNI: " + a.getDni() + ")"));
            }
            
            // Aquí es donde se llama al DAO para guardar
            boolean guardadoExitoso = eventoDAO.guardar(evento);
            
            if (guardadoExitoso) {
                System.out.println("Service: Evento '" + evento.getNombre() + "' agregado con éxito. ID: " + evento.getId());
            } else {
                System.err.println("Service: Fallo al agregar evento '" + evento.getNombre() + "'. Verifique los logs del DAO.");
            }
            return guardadoExitoso;
        } catch (Exception e) {
            System.err.println("Service: ERROR CRÍTICO al agregar evento '" + evento.getNombre() + "': " + e.getMessage());
    ...
    public boolean modificarEvento(Evento eventoModificado) {
        ...
        try {
            boolean modificadoExitoso = eventoDAO.guardar(eventoModificado); // Usa guardar() que maneja persist y merge
            if (modificadoExitoso) {
                System.out.println("Evento '" + eventoModificado.getNombre() + "' modificado con éxito.");
            } else {
                System.err.println("Fallo al modificar evento '" + eventoModificado.getNombre() + "'. Verifique los logs del DAO.");
            }
    ...
    public boolean registrarParticipanteAEvento(Evento evento, Participante participante) {
        ...
        try {
            ...
            // Forzar la inicialización de la colección de inscripciones antes de verificar si ya existe.
            // Esto evita LazyInitializationException si la colección no ha sido cargada.
            Hibernate.initialize(eventoGestionado.getInscripciones());
    ...
    public List<Pelicula> getTodasLasPeliculasDeCiclos() {
        ...
        // Abrir una nueva sesión para evitar LazyInitializationException si los eventos ya están detached
        EntityManager em = emf.createEntityManager();
        try {
            ...
            for (Evento evento : eventos) {
                if (evento instanceof CicloDeCine) {
                    ...
                    // Forzar la inicialización de la colección de películas
                    Hibernate.initialize(ciclo.getPeliculasAProyectar());
                    todas.addAll(ciclo.getPeliculasAProyectar());
                }
            }
        } catch (Exception e) {
            System.err.println("Error al obtener todas las películas de ciclos: " + e.getMessage());
            e.printStackTrace();
        } finally {
            em.close();
        }
